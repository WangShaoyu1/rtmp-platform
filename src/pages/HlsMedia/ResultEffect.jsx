...
const App = () => {
  ...
  const onFinish = async (values) => {
    ...
    // 确定音频平台和发音人
    const [platform, speaker] = values?.selectedAudio.split('-');
    console.log('platform:', platform, 'speaker:', speaker);
    let ttsGen = null;
    switch (platform) {
      case 'OpenAI':
        ttsGen = openaiTTSGen;
    ...
    // 生成音频
    const { data: audioFile } = await ttsGen({
      text: values.text,
      speaker,
      speed: 1,
    });
    ...
    formData.append(
      'audio',
      audioFile,
      `${values.text.slice(0, 3)}_${Date.now()}.mp3`,
    );
    formData.append('video', values.selectedVideo);
    formData.append('avatar_name', values.selectedVideo);
    // 提交数据
    axios({
      method: 'post',
      ...
      .catch((response) => {
        ...
        console.log('生成推理视频失败_result：', response);
      });
  };
  ...
  const handleSelectVideo = (video) => {
    ...
    form.setFieldValue('selectedVideo', video.name);
      ...
      <ProCard title={''} colSpan={'40%'}>
        ...
        <Form form={form} layout="horizontal" onFinish={onFinish}>
          <Form.Item
            label="输入文本"
            name="text"
            initialValue={'请跟着我第一遍，我们是神会主义接班人'}
            rules={[
              {
                required: true,
                message: '请输入文本',
          ...
          >
            <Input.TextArea placeholder="请输入文本" rows={4} />
          </Form.Item>
          <Row gutter={24}>
            <Col span={24}>
              {/* 模式选择 */}
              <Form.Item label="选择模式" name="handleType">
                <Select allowClear={true} onSelect={(value) => setMode(value)}>
                  ...
                  <Select.Option value="text-to-question">
                    文本提问
                  </Select.Option>
                </Select>
              </Form.Item>
          ...
          <Row gutter={24}>
            <Col span={24}>
              ...
              >
                <Select placeholder="选择一个音频" allowClear={true}>
        ...
        <Row gutter={[16, 16]}>
          {videoLikes.map((video, index) => (
            <Col span={8} key={index}>
              <div
                className={`result-video-container ${
